...
//		break;
	...
	case '=':
		if (demoState->demoMode == 0)
			toggleSong(demoState, 1);
		break;

	case '-':
		if (demoState->demoMode == 0)
			toggleSong(demoState, demoState->mSongCount - 1);
		break;
	...
	case 'u':
		if (demoState->mIter >= 0.05)
			demoState->mIter -= 0.05f;
		break;

	case 'i':
		if (demoState->mIter <= 14.95f)
			demoState->mIter += 0.05f;
		break;
	}
